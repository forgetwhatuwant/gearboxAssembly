...
from .sun_gear_3_env_cfg import SunGear3EnvCfg
...
from Galaxea_Lab_External.robots import SunGear3RulePolicy
from isaaclab.sensors import Camera

import h5py

class SunGear3Env(DirectRLEnv):
    cfg: SunGear3EnvCfg

    def __init__(self, cfg: SunGear3EnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self.rule_policy = SunGear3RulePolicy(sim_utils.SimulationContext.instance(), self.scene, self.obj_dict)
    ...
    def evaluate_score(self):
        ...
        # Check only first 3 gears
        gear_names = ["sun_planetary_gear_1", "sun_planetary_gear_2", "sun_planetary_gear_3"]
        
        if self.rule_policy.gear_to_pin_map is None:
            return 0
            
        for gear_name in gear_names:
            gear = self.obj_dict[gear_name]
            # Use the pre-calculated pin world position from rule policy if available
            # But calculating it dynamically is safer if carrier moves
            
            # Find which pin this gear was assigned to
            pin_idx = self.rule_policy.gear_to_pin_map[gear_name]['pin']
            pin_local_pos = self.pin_local_positions[pin_idx]
            
            # Ensure all tensors have matching batch dimensions for tf_combine
